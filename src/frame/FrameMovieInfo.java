...
public class FrameMovieInfo extends JPanel {
	...
	public FrameMovieInfo(Movie m) {
		...
		// 포스터 부분
		final int movieNum = 8; // 방갯수 만들어 놓음
		int movieflag = 0;

		String[] movieur1 = { "라라랜드2.png", "말할수없는비밀2.jpg", "인사이드아웃2.png", "겨울왕국2.png", "당신거기있어줄래요2.png", "스파이더맨2.png",
				"어벤져스2.png", "부산행2.png" };

		ImageIcon[] movieImg = new ImageIcon[movieNum];
		for (int i = 0; i < movieNum; i++) {
			movieImg[i] = new ImageIcon(movieur1[i]);
		...
		add(img);// 설정한 값 프레임에 붙이기

		// 영화 정보 텍스트(누르면 영화당 정보 주는거)
		...
		btnBack.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(0<=buttonNum && buttonNum<=3) {
					FrameBase.getInstance(new FrameMovieSelect()); //첫번째 페이지로 이동
				}else {
					FrameBase.getInstance(new FrameMovieSelect2());//두번쨰 페이지로 이동
				}
			}
		});//back버튼에 액션 입력
