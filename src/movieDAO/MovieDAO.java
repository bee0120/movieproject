package movieDAO;
//오라클에 보내주고 오라클에서 get/set할거 기능 정리
...
public class MovieDAO {
	...
	private void init() {
		...
		// 영화 정보2
		list.add(new Movie("말할수없는비밀", 10000, 5,
...
//--------------------------------------------------------------------------
	//예매정보 리스트에 방금 예매한 정보를 등록
	public void addReserveInfo(MyMovie e) { //현재 예약한 정보 확인 가능
		reservList.add(new MyMovie(e.getmName(),e.getTime(),e.getSeat())); //이벤트에서 이름,시간,좌석 정보 가져오기
	}
	
	//이미 예매한 정보인지 검색
	public boolean dupSeat(String name,String time,String seat) {
		for(int i=0; i<reservList.size(); i++) {
			if(reservList.get(i).getmName().equals(name) //이름
					&&reservList.get(i).getTime().equals(time) //시간
					&&reservList.get(i).getSeat().equals(seat)) { //좌석
				return true;
			}	
		}
		return false;
	...
	public void inputReview(
			...
			for(int i=0; i<list.size(); i++) {
				if(list.get(i).getmName().equals(mName)) {
					...
					list.get(i).setPersonScore(ScoreNum);//내가 남긴 평점
					list.get(i).setRev(reMsg);
					
					list.get(i).setPersonNum();
					list.get(i).setTotal(ScoreNum); //총 평점 합산(사람수,평점)
					list.get(i).setScore();
					list.get(i).setReview();//한줄평
